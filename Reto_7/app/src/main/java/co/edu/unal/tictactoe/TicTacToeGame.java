...
public class TicTacToeGame {
    ...
    // --- Serialización / deserialización del tablero para modo online ---

    /** Devuelve un String de 9 chars con el tablero actual (espacios, 'X' u 'O'). */
    public String toBoardString() {
        return new String(mBoard);
    }

    /** Carga el tablero desde un String de 9 chars (sin validación). */
    public void setBoardFromString(String boardStr) {
        if (boardStr == null || boardStr.length() != BOARD_SIZE) return;
        for (int i = 0; i < BOARD_SIZE; i++) {
            mBoard[i] = boardStr.charAt(i);
        }
    }

    /** Carga el tablero desde un String de 9 chars, validando los caracteres. */
    public void setBoardFromStringSafe(String boardStr) {
        if (boardStr == null || boardStr.length() != BOARD_SIZE) return;
        for (int i = 0; i < BOARD_SIZE; i++) {
            char c = boardStr.charAt(i);
            if (c != HUMAN_PLAYER && c != COMPUTER_PLAYER && c != OPEN_SPOT) {
                return; // inválido: no cambia el tablero
            }
        }
        setBoardFromString(boardStr);
    }

    /** Evalúa un tablero dado como String SIN alterar el estado actual. */
    public int checkForWinner(String boardStr) {
        if (boardStr == null || boardStr.length() != BOARD_SIZE) return 0;
        char[] backup = getBoardState();     // copia segura del estado actual
        setBoardFromString(boardStr);        // carga el string a mBoard
        int res = checkForWinner();          // usa la lógica existente
        setBoardState(backup);               // restaura el estado original
        return res;
    }

    /** Devuelve un tablero vacío "         " (9 espacios). */
    public static String emptyBoardString() {
        char[] b = new char[BOARD_SIZE];
        java.util.Arrays.fill(b, OPEN_SPOT); // ' '
        return new String(b);
    }
